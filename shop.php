...
.popup2{
  ...
  box-shadow:0 2px 5px rgba(0,0,0,0.2);
    ...
    <form method="GET">
    <?PHP $brand_query ="SELECT * FROM categories";
...
<ul class="ks-cboxtags">
  ...
  if(isset($GET['brands'])){
    ...
    ?>
    <li>
      <input   id="<?= $brandlist['check']; ?>"  type="checkbox" name = "brands[]" value = "<?= $brandlist['ID']; ?>" 
      <?php  if(in_array($brandlist['ID'], $checked)){ echo "checked"; }?>
      />
      <label for="<?= $brandlist['check']; ?>"><?= $brandlist['Name'];?></label>
</li>
    <?PHP
  }?><style>
    .container {
...
}?><button type="submit" class="filter" ><i class="fa fa-search"></i></button></ul></form></div>

<div  class="container2" style=" padding-left :35rem;">
...
<div class="col-md-2 ">
  ...
  <input type="text" name="end_price" value="<?php if(isset($_GET['end_price'])){ echo $_GET['end_price']; } else{ echo "500";}?>" class="form-control">
  </div>
  ...
  background-color:#3b98b7;
  border-radius:50%;
  padding-top:2px;"><i class="fa fa-search"></i></button>
  </div></div></form>
  </div>
...
<button class=" buy"><i class="fa fa-search" style="color:#fff;"></i> Quick View </button>
 ...
 if(isset($_GET['brands'])){
   ...
   foreach($branchecked as $rowbrand){
    $products = "SELECT * FROM items WHERE cat_ID IN ($rowbrand)";
